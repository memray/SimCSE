...
class Indexer(object):
    def __init__(self, vector_sz, n_subquantizers=0, n_bits=8, num_threads: int = 1):
    ...
    def to_gpu(self):
        ...
        print(f"Moving index to {faiss.get_num_gpus()} GPU")
        if faiss.get_num_gpus() == 1:
            res = faiss.StandardGpuResources()
            self.index = faiss.index_cpu_to_gpu(res, 0, self.index)
        else:
            # it takes very long time, but avoid OOM
